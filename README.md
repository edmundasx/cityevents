...
1. Paleiskite Apache ir MySQL per XAMPP.
2. Importuokite SQL struktūrą ir pavyzdinius duomenis:
   ```sql
   SOURCE /path/iki/projekto/api/schema.sql;
   ```
3. Jei reikia, pakoreguokite `api/config.php` (DB host, vartotojas, slaptažodis, DB pavadinimas).
4. Įkelkite projekto katalogą į `htdocs` (arba sukurkite „VirtualHost“) ir atidarykite `http://localhost/Miesto-Renginiai/`.
